...
import type { Question, RawQuestionInput } from './types';
...
function parseJSON(content: string, baseOrder: number): Question[] {
  const jsonData = JSON.parse(content);
  const questionsArray = Array.isArray(jsonData) ? jsonData : jsonData.questions || [];
  return questionsArray.map((q: RawQuestionInput, index: number) => ({
    ...
    question_order: baseOrder + index + 1,
  }));
}
...
export function useImportExport(questions: Question[], onImport: (questions: Question[]) => void) {
  ...
  const handleFileImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (fileName.endsWith('.csv')) {
        ...
        parsedQuestions = parseJSON(content, 0);
      } else {
  ...
  const handleManualImport = () => {
    try {
      ...
      if (trimmedInput.startsWith('[') || trimmedInput.startsWith('{')) {
        parsedQuestions = parseJSON(trimmedInput, questions.length);
  ...
  const downloadTemplate = () => {
    ...
    toast({
      ...
      description: `Đã tải file quiz-template.csv`,
    });
  };

  // Export functions — 11-col template layout
  const exportToCSV = () => {
    ...
    toast({
      title: "Export thành công",
      description: `Đã tải file questions.csv`,
    });
  };
  ...
  const exportToTXT = () => {
    ...
    toast({
      title: "Export thành công",
      description: `Đã tải file questions.txt`,
    });
  };
  ...
  const exportToJSON = () => {
    ...
    toast({
      title: "Export thành công",
      description: `Đã tải file questions.json`,
    });
  };
