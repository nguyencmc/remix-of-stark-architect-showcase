...
const log = logger('usePracticeEditor');
...
export interface PracticeEditorState {
  currentStep: number;
  saving: boolean;
  loading: boolean;
  title: string;
...
export interface PracticeEditorActions {
...
export function usePracticeEditor(): PracticeEditorState & PracticeEditorActions {
  const { id } = useParams();
  ...
  }, [user, isEditMode, id, navigate, fetchData]);
